...
def webhook():
    if request.method == "GET":
        ...
        if (
            ...
            and gitlab_message.get("object_attributes").get("state") == "opened"
            and gitlab_message.get("object_attributes").get("merge_status")
            == "checking"
        ):
            # Pass the verification and get the commit information
            log.info("First timemerge_request ", gitlab_message)
            project_id = gitlab_message.get("project")["id"]
            merge_id = gitlab_message.get("object_attributes")["iid"]

            thread = threading.Thread(
                target=review_code_for_mr, args=(project_id, merge_id, gitlab_message)
            )
            thread.start()

            return jsonify({"status": "success"}), 200
        elif object_kind == "push":
            merge_id = get_merge_request_id(
                gitlab_message.get("ref").split("/")[-1],
                gitlab_message.get("project_id"),
            )
            project_id = gitlab_message.get("project")["id"]
            if not merge_id:
            ...
            thread = threading.Thread(
                target=review_code_for_add_commit,
