...
def generate_review_note(change):
    try:
        ...
        review_note += (
            f'({total_tokens} tokens) {"LLM review got the result as following:"}'
            + "\n\n"
        )
        review_note += (
            response_content
...
def chat_review(
    index, project_id, project_commit_id, content, context, merge_comment_info
):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for change in content:
            if any(
                ...
                for ext in [".py", ".java", ".class", ".vue", ".go"]
            ) and not any(change["new_path"].endswith(ext) for ext in ["mod.go"]):
...
def review_code(project_id, project_commit_id, merge_id, context):
    ...
    for commit_id in project_commit_id:
        ...
        if response.status_code == 200:
            ...
            review_info += chat_review(
                index, project_id, commit_id, content, context, ""
            )

        else:
            log.error(
                f"Request gitlab的{url}commit failed, Status code:{response.status_code}"
            )
            raise Exception(
                f"Request gitlab的{url}commit failed, Status code:{response.status_code}"
            )
...
def review_code_for_mr(project_id, merge_id, gitlab_message):
    ...
    if changes and len(changes) <= maximum_files:
        ...
        log.error(
            f"Get merge_request information failed, project_id:{project_id} | merge_id{merge_id}"
        )
        raise Exception(
            f"Get merge_request information failed, project_id:{project_id} | merge_id{merge_id}"
        )
...
@retry(stop_max_attempt_number=3, wait_fixed=2000)
def review_code_for_add_commit(
    project_id, merge_id, commit_change_files, gitlab_message
):
    """
    ...
    change_files = [
        file_content
        for file_content in merge_change_files
        if file_content["new_path"] in commit_change_files
    ]
